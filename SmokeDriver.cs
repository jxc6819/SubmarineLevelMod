...
namespace IEYTD2_SubmarineCode
{
    ...
    {
        public SmokeDriver(IntPtr p) : base(p) { }
        public SmokeDriver() : base(ClassInjector.DerivedConstructorPointer<SmokeDriver>())
        ...
        public int SmokePool = 160;
        public float SpawnRate = 80f; //particles per second

        public float RiseSpeed = 0.9f; //upward velocity
        public float LateralSpread = 0.4f; //how much it mushrooms outwards

        public float LifeMin = 2.0f;
        public float LifeMax = 4.0f;

        public float SizeMin = 0.18f;
        public float SizeMax = 0.35f;

        public float Turbulence = 0.45f;
        public float TurbFrequency = 0.9f;

        public Color SmokeColor = new Color(0.05f, 0.05f, 0.05f, 0.95f);
        ...
        void Awake()
        {
            ...
            var sh = Shader.Find("FX/FX_Alpha_UVPan_Shader");
            if (!sh)
            ...
            SetAllColorProps(_mat, SmokeColor);
            ...
            _puffs = new Puff[SmokePool];
            for (int i = 0; i < SmokePool; i++)
                _puffs[i] = MakePuff("SmokePuff_" + i, _mat);
        }
        ...
        void Update()
        {
            ...
            for (int i = 0; i < _puffs.Length; i++)
            {
                ...
                float s = Mathf.Lerp(p.size, p.size * 1.8f, t);
                p.tr.localScale = new Vector3(s, s, 1f);

                float alpha = Mathf.Clamp01(1f - t);
                float density = Mathf.Clamp01(1.2f - t * 0.6f); //stays denser longer
                var col = SmokeColor;
                col.a = SmokeColor.a * alpha * density;
        ...
        void SpawnPuff(Vector3 origin)
        {
            ...
            Vector2 ang = UnityEngine.Random.insideUnitCircle * 0.4f;
            Vector3 dir =
                Vector3.up +
        ...
        Texture2D BuildRadialTex(int size)
        {
            ...
            for (int y = 0; y < size; y++)
            {
                for (int x = 0; x < size; x++)
                {
                    ...
                    float a = Mathf.Clamp01(1f - Mathf.SmoothStep(0.4f, 1.0f, r));
                    a = Mathf.Pow(a, 1.6f);
                    byte v = (byte)(a * 255f);
        ...
        static void SetAllColorProps(Material m, Color c)
        {
            ...
            if (m.HasProperty("_EmissionColor")) m.SetColor("_EmissionColor", c * 0.15f);
        }
